...
if [ -z "$__LEO_HOOKS_LOADED" ]; then
  ...
  _leo_precmd() {
    ...
    printf '\e]7;file://%s%s\e\\' "${HOSTNAME:-$(uname -n 2>/dev/null)}" "$(_leo_urlencode "$PWD")"
    if [ -z "$__LEO_PS1_INJECTED" ]; then
      PS1='\[\e]133;B\e\\\]'"$PS1"
      __LEO_PS1_INJECTED=1
    fi
    printf '\e]133;A\e\\'
  }
  ...
  case ":${PROMPT_COMMAND:-}:" in
    *":_leo_precmd:"*) ;;
    *) PROMPT_COMMAND="_leo_precmd${PROMPT_COMMAND:+;$PROMPT_COMMAND}" ;;
  esac

...
# zsh, so we shadow $? into `_leo_ret`.

{
  _leo_user_zdotdir="${LEO_USER_ZDOTDIR:-$HOME}"
  [ -f "$_leo_user_zdotdir/.zshrc" ] && source "$_leo_user_zdotdir/.zshrc"
...
if [[ -z "$__LEO_HOOKS_LOADED" ]]; then
  __LEO_HOOKS_LOADED=1
  ...
  _leo_precmd() {
    local _leo_ret=$?
    printf '\e]133;D;%s\e\\' "$_leo_ret"
    printf '\e]7;file://%s%s\e\\' "${HOST}" "$(_leo_urlencode "$PWD")"
  ...
  if (( $+functions[add-zsh-hook] )); then
    add-zsh-hook precmd _leo_precmd
    add-zsh-hook preexec _leo_preexec
  fi

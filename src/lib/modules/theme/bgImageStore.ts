// Images live in IndexedDB (Blobs); the UI references them by id.

const DB_NAME = 'leo-bg-images';
const STORE = 'images';
const VERSION = 1;
...
export async function putBgImage(id: string, blob: Blob): Promise<void> {
  ...
  try {
    ...
    if (e instanceof DOMException && e.name === 'QuotaExceededError') {
      throw new Error('Not enough storage to save this image. Remove unused backgrounds and try again.');
    }
    throw e;
  }
}
...
/** Returns an error message if `size` exceeds the cap for its kind, else null. */
export function sizeLimitMessage(size: number, animated: boolean): string | null {
  const limit = animated ? MAX_ANIMATED_BYTES : MAX_STATIC_BYTES;
  if (size <= limit) return null;
  const limitMb = Math.round(limit / 1024 / 1024);
  return animated
    ? `Animated images are limited to ${limitMb} MB to keep things smooth. This one is ${formatBytes(size)}.`
    : `Images are limited to ${limitMb} MB. This one is ${formatBytes(size)}.`;
}
...
export async function importBgImageFromFile(file: File): Promise<{ id: string; blob: Blob }> {
  ...
  const limitErr = sizeLimitMessage(file.size, animated);
...
async function encodeJpeg(bitmap: ImageBitmap, w: number, h: number): Promise<Blob> {
